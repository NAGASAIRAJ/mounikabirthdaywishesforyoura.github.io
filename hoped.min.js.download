...
function selected(){
	...
	//alert(BaseURL);
	$('#error-display').hide();
	var selected = $("#wishes-select option:selected").val();
...
function create(){
	...
	if(selected == '0'){
		error = '1';
		$('#error-display').html('Select Your Wishing Choise');
		$('#error-display').show();
		return false;
	}
	...
	if(selected == "birthday"){
		...
		bwords = $('#bwords').val();
		if(bname == ''){
			$('#error-display').html('Enter Birthday Person Name');
			$('#error-display').show();
			error = '1';
			return false;
		}else{
		bname = bname.replace(/[^A-Za-z ]/g, "");
		//bname = bname.replace(/ /g, '-');

		if(bwords != ''){
			bwords = bwords.replace(/[^A-Za-z ]/g, "");
			//bwords = bwords.replace(/ /g, '-');
			//window.location = BaseURL+selected+'/'+name+'/'+bname+'/?sp='+bwords;
			//return;
		}

		if(name == ''){
			$('#error-display').html('Please Enter Your Name');
			$('#error-display').show();
			error = '1';
			return false;
		}else{
	...
	window.location = BaseURL+selected+'/?n='+name+'/';	
	}
	...
	if(error == '0'){
		...
		type: "POST",
		url: BaseURL+'create/',
		data: {"name":name, "selected":selected, "bname":bname, "message":bwords},
